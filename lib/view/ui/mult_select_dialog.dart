...
class MultiSelectDialog<T> extends StatefulWidget with MultiSelectActions<T> {
  ...
  ///Funtion for initial value checking

  final bool Function(T value, List<T> values) checkingFunction;
  ...
  MultiSelectDialog({
    ...
    required this.checkingFunction,
    this.title,
...
class _MultiSelectDialogState<T> extends State<MultiSelectDialog<T>> {
  ...
  void initState() {
    ...
    for (int i = 0; i < _items.length; i++) {
      ...
      if (widget.checkingFunction(_items[i].value, _selectedValues)) {
        ...
        onChanged: (checked) {
          setState(() {
            _selectedValues = widget.onItemCheckedChange(
                _selectedValues, item.value, checked!, widget.checkingFunction);
...
mixin MultiSelectActions<T> {
  List<T> onItemCheckedChange(List<T> selectedValues, T itemValue, bool checked,
      bool Function(T value1, List<T> value2) checkingFuntion) {
    if (checked) {
      selectedValues.add(itemValue);
    } else {
      selectedValues.removeWhere(
        (element) => checkingFuntion(element, [itemValue]),
      );
    }
