...
class DataTable extends StatelessWidget {
  ...
  DataTable({
    ...
    this.clipBehavior = Clip.none,
        ...
        assert(
            columnWidthList == null || columnWidthList.length == columns.length,
            "Data width and column number not same"),
  ...
  ///use -1 to expanded
  final List<double>? columnWidthList;
  ...
  Widget build(BuildContext context) {
    ...
    final List<TableColumnWidth> tableColumns = <TableColumnWidth>[
          if (displayCheckboxColumn) const _NullTableColumnWidth()
        ] +
        (columnWidthList == null
            ? columns
                .map(
                  (e) => const _NullTableColumnWidth(),
                )
                .toList()
            : columnWidthList!.map((e) {
                return e == -1 ? FixedColumnWidth(e) : FixedColumnWidth(e);
              }).toList());
